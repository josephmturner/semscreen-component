...
export const messageReducer = (
  ...
  switch (action.type) {
    ...
    case Actions.setMainPoint:
      newState = handleSetMainPoint(state, action as Action<SetMainPointParams>);
      break;
    case Actions.combinePoints:
      newState = handleCombinePoints(state, action as Action<CombinePointsParams>);
      break;
    case Actions.splitIntoTwoPoints:
      newState = handleSplitIntoTwoPoints(state, action as Action<SplitIntoTwoPointsParams>);
      break;
  }
  return newState;
}

function handlePointCreate(state: MessageState, action: Action<PointCreateParams>): MessageState {
...
function handlePointUpdate(state: MessageState, action: Action<PointUpdateParams>): MessageState {
      return {
          ...state,
          points: {
            ...state.points,
...
function handlePointMove(state: MessageState, action: Action<PointMoveParams>): MessageState {
  ...
  if (
    state.focus &&
    action.params.pointId === state.focus.pointId
  ) {
    newFocus = undefined;
  }
...
function handlePointsDelete(state: MessageState, action: Action<PointsDeleteParams>): MessageState {
      return {
          ...state,
          points: allPointShapes.reduce(
            (obj: PointsI, pointShape: PointShape) => {
              obj[pointShape] = state.points[pointShape].filter(
                (p) => {
...
function handleSetFocus(state: MessageState, action: Action<SetFocusParams>): MessageState {
      const intermediateState = handlePointMove(state, action as Action<PointMoveParams>);
...
function handleSetMainPoint(state: MessageState, action: Action<SetMainPointParams>): MessageState {
      return {
        ...state,
...
function handleCombinePoints(state: MessageState, action: Action<CombinePointsParams>): MessageState {
...
function handleSplitIntoTwoPoints(state: MessageState, action: Action<SplitIntoTwoPointsParams>): MessageState {
      ...
      splitPoints.splice(
        ...
        { ...action.params.bottomPoint, pointId: action.params.newPointId, pointDate: new Date() }
      );
      return {
          ...state,
          points: {
